...
public struct Hotkey: Equatable, Sendable, CustomStringConvertible {
    ...
    /// Right Option + F1/F2 for the second one.
    ...
    private init(keys: String, action: Action, display: Target) {
        self.init(keys: try! KeyCombination(parsing: keys), action: action, display: display)
    }

    public var delta: Int { action == .up ? step : -step }

    /// `lopt+f1  main down 10`, the form `config show` prints.
    public var description: String {
