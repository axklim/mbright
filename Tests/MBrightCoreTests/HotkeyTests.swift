...
@Test func hotkeyDefaultsAreTheIssueProposal() throws {
    #expect(Hotkey.defaults == [
        Hotkey(keys: try parse("lopt+f1"), action: .down, display: .main),
        Hotkey(keys: try parse("lopt+f2"), action: .up, display: .main),
        Hotkey(keys: try parse("ropt+f1"), action: .down, display: .secondary),
        Hotkey(keys: try parse("ropt+f2"), action: .up, display: .secondary),
    ])
    #expect(Hotkey.defaults.allSatisfy { $0.step == 10 })
}
...
@Test func hotkeyDecodesDisplayKeywordsAndSelectors() throws {
    ...
    #expect(try decode(#"{"keys": "lopt+f1", "action": "down"}"#)
        == Hotkey(keys: try parse("lopt+f1"), action: .down, display: .main, step: 10))
...
@Test func hotkeyDescribesItselfForTheCLI() throws {
    #expect(Hotkey(keys: try parse("lopt+f1"), action: .down, display: .main).description == "lopt+f1  main down 10")
